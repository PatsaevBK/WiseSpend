...
import info.javaway.wiseSpend.features.settings.SettingsContract.Effect.DataSynced
import info.javaway.wiseSpend.features.settings.SettingsContract.Effect.Error
...
class SettingsComponentImpl(
    ...
    private fun onSyncOutput(output: SyncComponent.Output) {
        when (output) {
            SyncComponent.Output.DataSynced -> _effects.tryEmit(DataSynced)
            is SyncComponent.Output.Error -> _effects.tryEmit(Error(output.string))
        }
    }
